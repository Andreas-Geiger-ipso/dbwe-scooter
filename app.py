...
def login():
    if request.method == "POST":
        ...
        if user:
            ...
            session["username"] = user.username   # HIER rein
...
def scooters():
    ...
    if user_id:
        ...
        if active_booking:
            duration = int((datetime.now() - active_booking.start_time).total_seconds() / 60)
...
def stop_booking(booking_id):
    ...
    # Preis: z. B. 0.20 CHF pro Minute
    booking.price = round(duration * 0.2, 2)
